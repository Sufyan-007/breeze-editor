...
from ..utils.utils import process_route_config, include_all_routes_accessory_data, transform_route_config, rewrite_clean_route_config, handle_advance_prop_details
...
def add_route(request, project_id):
    ...
    try:
        ...
        if validate_route_path(data, data.get("parentId"), project_id) is True:    
            ...
            transform_route_config(config_data, node_id)
            ...
            # not to give transformed implemented function body but original string
            handle_advance_prop_details(config_data, {**request.data, 'id': node_id})
...
def update_route(request, project_id):
    ...
    try:
        ...
        # create clean and properly formatted config for code generation
        transform_route_config(config_data, data['id'])
        # now it isn't useful since we won't have a full path as a key
        # add_params_to_route_object()
        rewrite_clean_route_config(project_id, config_data, data['id'])
        process_route_config(project_id, config_data)
        # not to give transformed implemented function body but original string
        handle_advance_prop_details(config_data, request.data)
        include_all_routes_accessory_data(project_id, list(config_data.values()))
        return JsonResponse(config_data, status=200)
    except Exception as e:
        print("Error ", e)
