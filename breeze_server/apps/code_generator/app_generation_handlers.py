...
import subprocess
import json, os , uuid
from apps.common.utils.formatter import format_by_prettier,format_val
import pathlib
import shutil
import re
...
class AppGenerator:
    ...
    def read_configs(self):
        ...
        self.comp_config = read_project_config_file(self.app_config_dir, CONFIG_FILES_PATH['COMPONENT_CONFIG'])
        
        # Read component config from different files and prepare map of config for all
        self.prepare_comp_config()

        self.context_comp_config = read_project_config_file(self.app_config_dir, CONFIG_FILES_PATH['CONTEXT_COMPONENT_CONFIG'])
        self.reducer_config = read_project_config_file(self.app_config_dir, CONFIG_FILES_PATH['REDUCER_CONFIG'])
        self.redux_store_config = read_project_config_file(self.app_config_dir, CONFIG_FILES_PATH['REDUX_STORE_CONFIG'])
        self.app_config['MAPPINGS'] = {}
        self.app_config['CSS_CONFIG'] = read_project_config_file(self.app_config_dir, CONFIG_FILES_PATH['CSS_CONFIG'])
        
        self.project_name = self.app_config['name']
        # self.prepare_path_mappings() 
        # self.prepare_mapping_config()

        self.routing_config = read_project_config_file(self.app_config_dir, CONFIG_FILES_PATH['ROUTING_CONFIG'])
