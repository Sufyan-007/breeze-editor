export async function getComponents(project_id){
    const response = await (await fetch("http://localhost:8000/config-reader/get-global-components/",
...
export async function getHtml(project_id,component,html_id){
    const response = await (await fetch("http://localhost:8000/editor/get-global-html-config/",
