...
export default function ProjectRouting() {
  ...
  const [compRouteProps, setCompRouteProps] = useState({
    compProps: {
      ...Object.fromEntries(
        Object.entries(componentConfig.pages).map(([key, value]) => [
          [key],
  ...
  const handleRouteOffCanvas = (route, mode) => {
    ...
    route.component
      ...
      : setIsRouteWithAComponent(false);
  };
  ...
  const handleRouteObjectChange = (route, prop, event) => {
    ...
    if (routeMode !== "View") {
      if (typeof event === "string") {
        callBackFunction = event.replace(
          /\/\*[\s\S]*?\*\/|(?<=[^:])\/\/.*|^\/\/.*/g,
          ""
        );
      } else {
  ...
  const handleRouteProps = (prop, value) => {
    setDisplayRoute((prevState) => {
      if (value) {
        ...
        const { [prop]: _, ...restProps } = prevState.props;
        return {
          ...prevState,
          props: restProps,
        };
  ...
  const saveTheRoute = async () => {
    ...
    if (routeMode === "Edit" || routeMode === "Add") {
      if (routeMode === "Add") {
        if (
          !selectRef.current.getValue()[0] ||
          selectRef.current.getValue()[0].value === "none"
        ) {
        ...
        routeMode === "Edit" &&
        currentOffCanvasRoute.initialParentPath
      ) {
        let selectedParentValue = selectRef.current.getValue()[0];
        if (selectedParentValue) {
          displayRoute.newFullParentPath =
            selectedParentValue.value.fullPath || "none";
        }
        let childObj = {
          fullParentPath: currentOffCanvasRoute.fullPath.substring(
            0,
            currentOffCanvasRoute.fullPath.length -
              currentOffCanvasRoute.path.length
          ),
  ...
  useEffect(() => {
    if (routeMode === "Edit") {
      if (currentOffCanvasRoute.initialParentPath) {
        let requiredParent = routes.find(
          (eachRoute) =>
            eachRoute.fullPath ===
            currentOffCanvasRoute.fullPath.substring(
              0,
              currentOffCanvasRoute.fullPath.length -
                currentOffCanvasRoute.path.length
            )
                    ...
                    >
                      <Form.Control
                        ...
                        style={{
                          position: "relative",
                          top:
                            routeMode === "childView" || routeMode === "Add"
                              ? "16px"
                              : "18px",
                          paddingBottom: "25px",
                            ...
                            <div className="accordion mt-2" id="componentsProp">
                              <div className="accordion-item">
                                <h2 className="accordion-header">
                                  ...
                                  >
                                    <span className="me-1">Props</span>
                                    <OverlayTrigger
                                      placement={"right"}
                                      overlay={
                                        <Tooltip id={`tooltip-prop-info`}>
                                          Insert Value to include prop & remove
                                          value to delete prop.
                                        </Tooltip>
                                      }
                                    >
                                      <i
                ...
                <div>
                  {isRouteWithAComponent && (
                    <div className="d-flex mt-1">
                      {showSelectedRouteObj.HydrateElement && (
                        <div className="mx-1">
                      ...
                      {showSelectedRouteObj.ErrorElement && (
                        <div className="mx-1">
                  ...
                  {showSelectedRouteObj.Action && (
                    <div className="my-3">
                      ...
                      <div className="mx-1 mb-1">
                        <MonacoEditor
                          defaultValue={
                            displayRoute.action || "/* Add action callback */"
                          }
                          onChange={(e) =>
                  ...
                  {showSelectedRouteObj.Loader && (
                    <div className="my-3">
                      ...
                      <div className="mx-1 mt-1 mb-3">
                        <MonacoEditor
                          defaultValue={
                            displayRoute.loader || "/* Add loader callback */"
                          }
                          onChange={(e) =>
                  ...
                  {showSelectedRouteObj.Lazy && (
                    <div className="my-3">
                      ...
                      <div className="mx-1 mt-1 mb-3">
                        <MonacoEditor
                          defaultValue={
                            displayRoute.lazy || "/* Add lazy callback */"
                          }
                          onChange={(e) =>
                  ...
                  {showSelectedRouteObj.ShouldRevalidate && (
                    <div className="my-3">
                      <div>
                      ...
                      <div className="mx-1 mt-1 mb-3">
                        <MonacoEditor
                          defaultValue={
                            displayRoute.shouldRevalidate ||
                            "/* Add shouldRevalidate callback */"
                          }
                          onChange={(e) =>
                            handleRouteObjectChange(
                              displayRoute,
              ...
              {routeMode !== "View" && (
                <div className="my-4">
                  ...
                  {routeMode !== "Add" && (
                    <div
                      className={`${
                        displayRoute ? "" : "disableRouteButton"
                      } btn btn-danger mx-1`}
                      data-bs-dismiss="offcanvas"
                      onClick={() => {
              ...
              {routes.map((route) => (
                <tr className="text-center" key={route.fullPath}>
                  ...
                  <td
                    width={"20%"}
                    title={`${route.path}${
                      route.initialParentPath ? " - is a Child Route" : ""
                    }`}
                  >
                    {route.path}
                    {route.initialParentPath ? " (CR)" : ""}
                  </td>
                  <td
                    width={"20%"}
                    title={route.component ? route.component : route.redirectTo}
                  >
                    {route.component ? (
                      route.component
                    ) : (
