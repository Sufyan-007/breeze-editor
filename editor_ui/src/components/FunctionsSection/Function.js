...
export default function Function({ func, updateFunctions }) {
    ...
    return (
        <div className={" container-fluid  border p-2 " + (showDetails ? "bg-dark" : "")}>
            ...
            <CommonConfigSidebar title={func.name} onClose={() => setShowDetails(false)}>
                {showDetails &&
                    <>
                        <div className="row mx-2" >
                            ...
                            <div className="col">
                                <textarea className=" w-75" value={functionConfig.description} onChange={(event) => setFunctionConfig(state => { return { ...state, description: event.target.value } })} placeholder="Description" name="" id=""></textarea>
                            </div>
                        </div>
                        ...
                        <div className="row mx-2 my-1 " >
                            <div className="col">
                                ...
                                {functionConfig.parameters.list.map((param, index) =>
                                    <div className="row">
                                        <FunctionParams param={param} setParam={(param) => updateParams(param, index) } />
                                    </div>
                                )
                                }
                                <div className="row">
                                    <div className=" m-1">
                                        {newParam !== null &&
                                            <input type="text" value={newParam} onChange={updateNewParam} className=" p-0 mx-2 " />
                                        }
                                        <button className="p-1 btn btn-sm btn-secondary" onClick={addParam}>
                                            Add Param
                                        </button>
                                    </div>
                                </div>
