...
class IntermediateConversion:
    ...
    def _create_parameters(self, url_data):
       ...
       parameters = [
                            Parameter(param_in="query", name=param.get("key"), type="string", required=True, description="") 
    ...
    def _create_body(self, body_data):
        ...
        for item in formdata_data:
            formdata_list.append(Formdata(key=item.get("key"), value=item.get("value"), description=item.get("description"), type=item.get("type"), src=item.get("src")))
