...
def generate_code(children):
    code=""
    for child in children:
        print(child["statementType"])

        if child["statementType"] == "STATEMENTS":
            child["code"] = generate_code(child.get("children",[]))
        elif child["statementType"] == "SINGLE":
            if type(child["code"]) is list:
                child["code"] = ''.join(child["code"])
        elif child["statementType"] == "WRAP":
            child["code"]= child["prefix"] + generate_code(child.get("children",[])) + child["suffix"]
...
def get_code_index(statements,code):
    ...
    for statement in statements:
        ...
        if indexes ==None:
            raise Exception()
        obj = {k:statement[k] for k in statement if k!="code" }
        if children:
            childCode= code[indexes[0]:indexes[1]+1]
            obj["children"] = get_code_index(children,childCode)
        tree.insertElem(indexes[0],indexes[1]+1,obj)
    return tree