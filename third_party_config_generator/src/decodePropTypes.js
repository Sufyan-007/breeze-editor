...
function decodePropType(prop, type, processed = [], recLevel=0, all=[]) {
    ...
    // if ([SyntaxKind.BooleanKeyword, SyntaxKind.StringKeyword, SyntaxKind.NumberKeyword].includes(prop.getTypeNode().getKind())) {
    //     // // console.log('BOOLEAN FOUND');
    //     typeInfo['type'] = 'DATA_TYPE'
    //     typeInfo['data'] = prop.getTypeNode().getText()
    // }
    if(typeof type.getKind == 'function' && type.getKind() == SyntaxKind.UnionType){
        typeInfo['type'] = 'UNION_TYPE'
        typeInfo['name'] = type.getText()
        typeInfo['data'] =  type.getTypeNodes().map(tn => decodePropType(prop, tn.getType(), processed, recLevel, all))
    }
        ...
        typeInfo['data'] = decodePropType(prop, type.getArrayElementType(), processed, recLevel, all)
    }
    else if(type.isAny()){
        typeInfo['type'] = 'DATA_TYPE'
        typeInfo['data'] = 'any'
        ...
        typeInfo['data'] = decodePropType(prop, type.getDefault(), processed, recLevel, all )
    }
        ...
        // else
         if (processed.includes(objectPath)){
            ...
            typeInfo['data'] = type.getProperties().map(p => {
                ...
                return {
                    name : p.getName(),
                    type : decodePropType(prop, p.getValueDeclaration().getType(), processed, recLevel, all)
                }
            })
        }
                    ...
                    if(types.getKind() == SyntaxKind.UnionType){
                        ...
                        for(const ut of types.getTypeNodes()){
                            ...
                            // if(type.constructor.name == 'Expression'){
                                type = type.getType()
                            // }
                            typeInfo['data'].push(decodePropType(prop, type, processed, recLevel, all))
                        }
                    }else{
                        for(const ut of type.getUnionTypes()){

                            typeInfo['data'].push(decodePropType(prop, ut, processed, recLevel, all))
                        }
                    }
                }
        ...
        typeInfo['data'] = { 
                returnType : {
                    ...
                    type : decodePropType(prop, callSign.getReturnType(), processed, recLevel, all)
                },
        ...
        if (processed.includes(objectPath)){
            ...
            // if(path){
                processed.push(objectPath)
            // }
            return decodePropType(prop, type.getSymbol()?.getDeclaredType(), processed, recLevel, all)
        }
    }else{
